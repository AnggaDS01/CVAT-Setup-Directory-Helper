...
def coco_json_process_video_pipeline(project_path, coco_json_filename, video_path, split_ratio=0.8, random_split=True, is_split=True, ext='jpg'):
    ...
    # 3. Split dataset
    if is_split:
        coco_json_split(project_path, coco_json_filename, split_ratio, random_split)
    else:
